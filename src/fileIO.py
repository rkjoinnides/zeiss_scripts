...
# Create an ini file using dict and path
def create_config_file(config_file_path: str, data: dict):
    """Creates a config file (if one does not already exist) in the
    given path destination with the entries in data.
    
    Args:
        config_file_path (str): path to file (including name)
        data (dict): 2 deep dictonary containing data to write.
    """

    if config_file_path.split(".")[-1] != "ini":
        print("Config file path must include .ini extension!")
...
if __name__ == "__main__":
    test_dict = {"TestSection": {"test1": "a", "test2": "b"}}
    create_config_file(os.getcwd() + "\\testConfig", test_dict)
>>>>>>> 4c4174113063601238f71b9034d4ae7d5f4416da
